...
result_t getHealth(device_health * health, uint32_t timeout) {
	...
	while ((time(&current_time) - currentTs) <= timeout) {
		uint8_t currentbyte;
		HAL_UART_Receive(&huart3, &currentbyte, 1, 10);
...
result_t sendCommand(uint8_t cmd, const void * payload, size_t payloadsize) {
	...
	HAL_UART_Transmit(&huart3, (uint8_t *)header, 2, 10);
	if((cmd & LIDAR_CMDFLAG_HAS_PAYLOAD)){
		...
		HAL_UART_Transmit(&huart3, &sizebyte, 1, 10);
		HAL_UART_Transmit(&huart3, (uint8_t *)payload, sizebyte, 10);
		HAL_UART_Transmit(&huart3, &checksum, 1, 10);
	}
	return RESULT_OK;
}
